...
function cheat.startplugin()
	...
	local function load_hotkeys()
		...
		local file = io.open(lfs.env_replace(manager:machine():options().entries.cheatpath:value():match("([^;]+)")) .. "/" .. cheatname .. "_Custom.json", "r")
		if not file then
			return
		end
	...
	local function save_hotkeys()
		...
		if #hotkeys > 0 then
			...
			local file = io.open(path .. "/" .. cheatname .. "_Custom.json", "w+")
			if file then
