...
int configuration_manager::load_settings()
{
	...
	if (filerr == osd_file::error::NONE)
		load_xml(file, config_type::GAME);

	/* finally, load the game-specific file */
	filerr = file.open(machine().basename() + ".cfg");
	osd_printf_verbose("Attempting to parse: %s.cfg\n",machine().basename());
	if (filerr == osd_file::error::NONE)
		loaded = load_xml(file, config_type::DEFAULT);
...
int configuration_manager::save_xml(emu_file &file, config_type which_type)
{
	...
	if (!systemnode)
		return 0;
	systemnode->set_attribute("name", (which_type == config_type::GAME) ? "default" : machine().system().name);
