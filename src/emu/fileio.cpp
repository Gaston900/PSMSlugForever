...
int checksum_file_crc32(const char *searchpath, const char *file, u8 **p, u8 *size, unsigned int *crc)
{
	u64 length;
	u8 *data;
	...
	if (f.read(data, (u32)length) != (u32)length)
	{
		free(data);
		f.close();
		return -1;
...
void GetDirectory(void* filename, char* dst, int dstlen)
{
	...
	_splitpath((const char *)filename, drive, dir, nullptr, nullptr);
	_makepath(fullpath, drive, dir, nullptr, nullptr);
