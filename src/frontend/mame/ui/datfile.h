...
public:
	...
	void datafile_exit();
	void datafile_init(running_machine &machine);

	// actions
	void load_data_info(const game_driver *drv, std::string &buffer, int type);
	bool load_command_info(const game_driver *drv, std::vector<std::string> &menuitems);
...
private:
	...
	bool index_menuidx(fileptr &&fp, const game_driver *drv, dataindex &idx, std::vector<std::string> &menuitems);
	...
	std::unique_ptr<emu_file>  m_file = nullptr; // current file
};
