...
#define CMD_LIST
#define ARRAY_LENG(x)     (sizeof(x) / sizeof(x[0]))
...
extern u8 cmd_sub_menu(const game_driver *drv, const char *menu_item[]);

menu_command::menu_command(mame_ui_manager &mui, render_container &container, bool menuless_mode) : menu(mui, container)
{
...
void menu_command::populate(float &customtop, float &custombottom)
{
	const char *item[256];
	int64_t menu_items;

	int64_t total = cmd_sub_menu(&machine().system(), item);

	if (total)
	{
		for (menu_items = 0; menu_items < total; menu_items++)
			item_append_ex(item[menu_items], "", 0, (void *)menu_items);
	}
	else
	{
		item_append_ex(_("There is no Command.dat file or Command Data."), "", 0, NULL);
	}
}
...
void menu_command::handle()
{
	...
	{
//		menu::stack_push(auto_alloc_clear(ui(), menu_command_content(ui(), container(), int((long long)(menu_event->itemref)) ) ) );
		menu::stack_push<menu_command_content>(ui(), container(), int((long long)(menu_event->itemref)));
	}
}

uint32_t menu_command::ui_handler(render_container &container, mame_ui_manager &mui)
{
	uint32_t result;

	// if this is the first call, push the menu
	if (topmost_menu<menu_command>(mui.machine()) == nullptr)
		menu::stack_push<menu_command>(mui, container, true);
	...
	if(topmost_menu<menu_command_content>(mui.machine()) != nullptr)
	{
		return 0;
	}
	else
	{
		menu_command *uim = topmost_menu<menu_command>(mui.machine());
		return uim && uim->m_menuless_mode ? 0 : UI_HANDLER_CANCEL;
	}
}


menu_command_content::menu_command_content(mame_ui_manager &mui, render_container &container, int _param) : menu_command(mui, container, false)
{
	param = _param;
}

menu_command_content::~menu_command_content()
{
}

void menu_command_content::handle()
{
	/* process the menu */
	process(PROCESS_CUSTOM_ONLY);
}
	
void menu_command_content::populate(float &customtop, float &custombottom)
{
	std::vector<int> xstart, xend;

	char commandbuf[64 * 1024]; // 64KB of command.dat buffer, enough for everything

	bool game_paused = machine().paused();

	/* Disable sound to prevent strange sound*/
	if (!game_paused)
		machine().pause();

	if (load_driver_command_ex(&machine().system(), commandbuf, ARRAY_LENG(commandbuf), (u32)param) == 0)
	{
		convert_command_glyphs(commandbuf,ARRAY_LENG(commandbuf));	

		//  FILE *fp;
		//  fp = fopen("commandbuf.txt", "a+t");
		//  fprintf(fp,"%s\n",commandbuf);
		//  fclose(fp);
...
menu_scale_effect::menu_scale_effect(mame_ui_manager &mui, render_container &container) : menu(mui, container)
{
}
...
void menu_scale_effect::handle()
{
	...
	/* process the menu */
	if (menu_event != NULL && menu_event->iptkey == IPT_UI_SELECT && 
		(uintptr_t)menu_event->itemref >= SCALE_ITEM_NONE)
	{
		screen_device *screen = screen_device_iterator(machine().root_device()).first();
		screen->video_exit_scale_effect();
		scale_decode(scale_name((uintptr_t)menu_event->itemref - SCALE_ITEM_NONE));
		screen->video_init_scale_effect();
		changed = true;
		osd_printf_verbose(_("scale effect: %s\n"), scale_name((uintptr_t)menu_event->itemref - SCALE_ITEM_NONE));
	}
...
{
	int scaler;
	item_append(_("None"), "", 0, (void *)SCALE_ITEM_NONE);
	...
	for (scaler = 1; ; scaler++)
	{
		...
		item_append_ex(desc, "", 0, (void *)(uintptr_t)(SCALE_ITEM_NONE + scaler));
	}
