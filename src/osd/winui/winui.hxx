...
public:
	virtual void output_callback(osd_output_channel channel, const util::format_argument_pack<std::ostream> &args) override
	{
		...
		{
			FILE *pFile;
			pFile = fopen("Emulator/MSForever/verbose.log", "a");
			fputs(buffer, pFile);
			fflush(pFile);
			fclose (pFile);
			return;
...
//			chain_output(channel, msg, args);   // goes down the black hole
		// LOG all messages
		FILE *pFile;
		pFile = fopen("Emulator/MSForever/winui.log", "a");
		fputs(buffer, pFile);
		fflush(pFile);
		fclose (pFile);
	}
...
int MameUIMain(HINSTANCE hInstance, LPWSTR lpCmdLine)
{
	...
	unlink("Emulator/MSForever/winui.log");
	unlink("Emulator/MSForever/verbose.log");
...
static bool MameCommand(HWND hWnd, int id, HWND hWndCtl, UINT codeNotify)
{
	...
	case ID_NOTEPAD:
		...
		case ID_HELP_WHATS_NEW :
			ShellExecuteCommon(hMain, ".\\Emulator/MSForever/docs\\What's new.txt");
			SetFocus(hWndList);
			return true;
